/*
  gen-object - создает запись об объекте
...
env.onvalue(0,(tname) => {
  ...
  r.params.manual_features = tname;
  env.setParam( 'output', r )
})
"

/*
  add-params - добавляет параметры в объект
...
/*
  add-child - добавляет 1 или более детей в описание объекта
...
feature "add-child" "
  ...
  env.onvalues_any( ['input',0,1],(input,c,cname) => {
    ...
    for (let cc of c) {
      let fname=cname;
      while (output.children[ fname ])
      fname = cname + '_' + (n++).toString();
      // console.log('computed cname=',cname, 'c=',c)
      output.children[ fname ] = cc
    }
    env.setParam( 'output', output )
  })
"