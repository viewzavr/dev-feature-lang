...
register_feature name="layers_gui3" {
          ...
          //body_features={set_params style="padding:0.2em 0.2em 0.2em 0.4em; gap: 0.2em;";}
  {
    ...
    find-objects pattern=@lgui->pattern 
     | repeater {
        collapsible2 text=@.->inputIndex body_features=@lgui->each_body_features
          active=(@.->input | get_param name="active")
        {{
          /*
          onevent name="user-changed-active" {
            setter object="@col->input" param="active"; // тут надо проработать передачу арг от событий
          }
          */
