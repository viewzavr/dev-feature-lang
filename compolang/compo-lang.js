  ...
  function recreate( probable_list, force=false ) {
     ...
     if (model.length < current_state.length) {
       ...
       for (let i=current_state.length; i<model.length; i++) {
           ...
           $scopeFor.$lexicalParentScope = env.$scopes.top();
           if (env.params.use_outer_scope)
             $scopeFor.$lexicalParentScope = env.$scopes[ env.$scopes.length - 2 ]
...
export function template( env, options )
{
  ...
  env.restoreChildrenFromDump = (dump, ismanual) => {
    ...
    if (dump.children_env_args) 
      arr.env_args = dump.children_env_args;
...
// преобразует дамп в дамп с отметками что это "ручной код"
// таким образом он будет создаваться при восстановлении состояния программы
// подходит для командных вещей. нажали кнопочку - сгенерили ручной код за пользователя.
export function dump_to_manual( env )
{
  env.onvalue("input",(dump) => {
...
// read @val | pass-input-if (cond..) default=@other-val;
...
// модификатор
// организует так что объект попадает в дамп (не путать с manual!)
...
// модификатор
// он спорный - т.к. требует хранить $vz_features_from_dump
/*
export function mark_manual(env) {
  env.on("attach",(obj) => {
      //obj.params.force_dump=true;
      obj.manuallyInserted=true;
      let orig = obj.dump;
      obj.dump = function() {
         if (obj.$vz_features_from_dump) {
           let s = Object.keys( obj.$vz_features_from_dump ).filter( f => f != "base_url_tracing" )
           obj.setParam("manual_features",s,true);
         }
         else
         {
           // ну попробуем хоть так..
           let list = {"vzf-object-uniq-ids":1,"viewzavr-object":1,"vzf-manual-features":1,"viewzavr-object-translate-text":1,"base-url-tracing":1,"connect-params-to-events":1};
           let s = Object.keys( obj.$features_applied ).filter( f => !list[f] );
           obj.setParam("manual_features",s,true);
         };  
         return orig();
      };
...
// вход: описание в стиле { |args| .... }, и позиционные аргументы
