...
Язык Компаланг позволяет описать древовидный набор объектов, их параметры, и связи между ними. 

Текущая версия интепретации создает описанные на языке объекты в памяти, и организует между ними передачу параметров по мере их изменения (в "реактивном" стиле).

Язык в текущей практике применяется для описания:
- сцен 3д графики
- конвейеров преобразования данных
- пользовательских интерфейсов, взаимодействия с пользователем
...
```
Если значение по ссылке меняется, то оно копируется в указанный параметр.
...
```
Объекты из такого набора не создаются системой автоматически, их программный код записывается в значение параметра.
Объекты, получившие такие коды в параметрах, могут пользоваться ими по своему усмотрению, например создавать описанные в них объекты по мере необходимости.

## Локальные параметры набора объектов
При описании набора объектов можно указать их локальные параметры. Они описываются как `|имя1 имя2 ...|` Пример:
```
read @arr | repeater { |sigma|
   text @sigma
}
```
Пример:
```
visual-object scene3d={ |renderer opacity|
  points positions=(load-file "alfa.json" | parse-json) opacity=@opacity
}
```
Здесь пользователь объекта может создать объекты из описания scene3d, передав им значения для renderer и opacity, которые будут доступны внутри области {} по указанным именам.
...
Для работы ссылок используется лексическая область видимости:
* В рамках исходного текста видны все идентификаторы объектов (`метка:`), см пример
* Параметры опубликованные let
* Локальные параметры набора объектов
...
Введен тип `m-lambda`, он записывает в параметр output указанную функцию. Если ему заданы позиционные параметры, то происходит "каррирование" и эти параметры становятся первыми в списке для передачи в функцию. 
Пример:
```
