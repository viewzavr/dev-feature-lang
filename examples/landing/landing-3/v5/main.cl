...
addons_place: a=5; //  todo fix visualhub:;

debugger_screen_r;

register_feature name="visual_layer" {
  vlayer: gui_title=( @t1 | get child=@selected_show->value | get param="title")
  {
    ...
    deploy_many_to target=@r1 
       input=( @t1 | get child=@selected_show->value | get param="render3d-items" )
    ...
    deploy_many_to target=@extra_screen_things 
       input=( @t1 | get child=@selected_show->value | get param="screen-items" )
       include_gui_from_output 
       {{ keep_state }}; // keep_state сохраняет состояние при переключении типов объектов   
