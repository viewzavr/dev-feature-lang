...
m_eval "() => scope.arr = [1,3,5]";

screen auto_activate {
  column gap="0.2em" style="width:160px" {
    text "privet";
    ...
    b: button "click" on_click=(m_lambda "() => {
      console.log('arr was', scope.arr );
      scope.arr = [3,2,3];
    }");
    
    b: button "click 2" on_click=(m_lambda "() => {
      console.log('arr was', scope.arr );
      //scope.arr.push( Math.random() );
      scope.arr = scope.arr.concat( [Math.random()] );
      // ну пусть так пока
    }");    
