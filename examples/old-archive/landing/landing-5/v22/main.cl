...
mainscreen: screen auto-activate {
  ...
  { 
    ...
    column padding="0.3em" margin="0.7em" gap="0.5em"
    {
      ...
      register_feature name="q_render_layers" {
        main: column gap="5px" {
          find-objects-bf features="layer_v1" root=@main->root debug=true
          | console_log text="ZAZAZA"
          | repeater {
              rt: column style="background: rgba(99, 116, 137, 0.36);padding: 5px; border-radius: 5px;" {
                text text=(@rt->input | get_param name="title");
