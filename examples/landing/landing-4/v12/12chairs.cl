        ...
        find-objects-bf root=@col->input pattern="include_gui_from_output"
           | repeater {
               subr: column { // здесь input это каждый найденный объект в полях output
                  ...
                  @subr->input | get param="output" | repeater {
                        find-objects-bf root=@.->input features="include_gui"
...
register_feature name="layers_gui" {
  ...
  lgui: column target=@. {
      ...
      myobjects: find-objects-bf features=@lgui->pattern root=@/;
  };
