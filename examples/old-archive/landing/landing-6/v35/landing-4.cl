...
feature "render-data-input" {
  r: row padding="0.2em" { 
     text "Данные: ";
     combobox
       name="input_link" 
       values=(@r->input | get_param "data_link_values")
       titles=(@r->input | get_param "data_link_titles")
       value=(@r->input | get_param "input_link")
       {{
        x-on "user_changed_value" {
          lambda @r->input code="(tobj,cbobj,val) => { 
            console.log('->',tobj,val)
            tobj.setParam('input_link',val) 
          }";
          //setter object=@r->main name="data_link" value=
        };
       }}
  };
};
...
linestr: feature {
  main: linestrips datavis 
     gui={ 
       render-data-input input=@main;
       render-params input=@main;
      };
};

ptstr: feature {
  main: points datavis gui={ 
    render-data-input input=@main;
    render-params input=@main;
    };
};
...
models: feature {
    root: node3d datavis gui={
      render-data-input input=@root;
