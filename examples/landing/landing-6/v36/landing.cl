...
view1: feature text="Общий вид" { 
  ...
  find-objects-bf root=@extra_screen_things features="screenvis" 
      | x-modify { 
      x-set-params
       time=@vroot->time 
...
view2: feature text="Ракета в центре координат" { 
  ...
  find-objects-bf root=@r1 features="datavis" 
      | x-modify { 
      x-set-params
       data_link_values = ["@dat_cur_time_zero->output","@dat->output","@dat_prorej->output","@dat_cur_time->output"]
       data_link_titles = ["Текущее время, точка O","Траектория","Прореженная","Текущее время"]
       ;
    };
  ...
  find-objects-bf root=@extra_screen_things features="screenvis" 
   ...
   ////////////////////////////////////
          ...
          target=@v1 {{ skip_deleted_children }}
    {
        ...
        models input_link="@dat_cur_time_zero->output" radius=1;
   ...
   render_interface
       ...
       right={
        render_layers title="Визуальные объекты" 
           root=@vroot
           items=[ {"title":"Объекты данных", "find":"datavis","add":"linestr","add_to": "@r1->."},
                   {"title":"Статичные","find":"staticvis","add":"axes","add_to": "@r1->."},
                   {"title":"Текст","find":"screenvis","add":"curtime","add_to": "@extra_screen_things->."}
                 ];
       };
