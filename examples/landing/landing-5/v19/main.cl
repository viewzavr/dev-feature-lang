...
load files="lib3dv3 csv params io gui render-params df scene-explorer-3d 12chairs.cl gui2.cl";
...
mainscreen: screen auto-activate {
  ...
  column style="" 
    ...
    layers_gui2 layer={data_visual_layer} pattern="** data_visual_layer" text="Визуализация" plashka;

    layers_gui2 layer={static_visual_layer} pattern="** static_visual_layer" text="Прочие образы" plashka;
...
register_feature name="data_visual_layer" {
  dv: active=true gui_title=(@pc->titles | get name=@pc->index) 
    //gui_title=@al->gui_title
    //(@pc->titles | get name=@input_data->index) 
    {
