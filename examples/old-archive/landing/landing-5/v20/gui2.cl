    ...
    find-objects pattern=@lgui->pattern pattern_root=@lgui->pattern_root
     | repeater {
        ...
        }}
        {
          ...
          // здесь получается мы рисуем содержимое гуи одного слоя
          // именно тут по логике нам и надо менять тип отображения гуи в зависимости от того что предложит слой
          // по сути мы обопремся на некую таблицу, когда нам объект слоя расскажет, какого типа гуи он советует применить
          if condition=(@coco->input | has_feature name="use_layered_gui")
          {
            solution_gui sol=@coco->input;
            render-guis-nested2 input=@coco->input {{ console_log_params text="RERERE"}};
          };
