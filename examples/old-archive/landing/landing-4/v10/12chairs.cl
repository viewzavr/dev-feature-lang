...
register_feature name="addon_layer" {
    ...
    items_in_list=(@.->list | get_children_arr | arr_filter code=`(c) => c.params.title`)
  {
    ...
    | repeater {
      ...
      { 
        dmt: deploy_many_to target=@recroot->target
           ...
           ;
        // setter target=(@dmt->output | get name=0) param="input" value=@vlayer->input auto_apply;
        // передача input-а
      };
    };
...
// вход: target - куда добавлять новое
//       layer  - описание нового
//       titile - заголовок
