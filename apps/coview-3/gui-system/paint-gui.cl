...
/*
dom-comp "gui-float-table" { |in out|
	...
	gui-text 
	  (read @in | get-value | arr2text | create-channel)
		(z: create_channel)

		{{ read @z->output | get-value | text2arr | put-value-to @out }}	
...
// хитрая штука. на вход выход - одномерный массив. но редактор - по колонкам.
// используется для редактирования "упакованных массивов" типа координат, цвета
...
// короче сделал пока вторым параметром, необязательным. но вопросы отсечения остаются.
feature "param-info" {
	x: object object=@.. name=@.->0 in=false out=false
				path=(param-path @x.object @x.name)
				//{{ reaction existing=true @x.1? (param @x.object @x.name) }}
				{{ reaction existing=true @x.1? {: val obj=@x.object name=@x.name |
					   if (obj.paramConnected( name )) return
