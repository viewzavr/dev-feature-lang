...
register_feature name="data_visual_layer" {
  dv: active=true gui_title=(@pc->titles | get name=@pc->index) 
    ...
    pc: param_combo name="input_data" 
          titles=["Траектория","Прореженная траектория","Текущее положение","Текущее положение в нуле"]
          values=["@dat->output","@dat_prorej->output","@dat_cur_time->output","@dat_cur_time_zero->output"]
     ;
