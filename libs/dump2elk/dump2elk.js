...
function add_ports_if_needed( path, objdump )
{
  ...
  function findobj( objid, rootdump ) {
    ...
    let cc = Object.values( rootdump.children || {} ).concat( Object.values( rootdump.features_list || {} ) ).concat( Object.values(rootdump.params_env_list || {}).flat(1) );;
...
function fix_links( objdump ) {
  ...
  if (!objdump.$elk)
    console.warn("objdump.$elk is null",objdump)
  ...
  let cc = Object.values( objdump.children || {} ).concat( Object.values( objdump.features_list || {} ) ).concat( Object.values(objdump.params_env_list || {}).flat(1) );
  //console.log( "ccc=",cc,"ccc finish" )
...
function gen( objdump, parent_path="", parent_object, is_hosted, skip_links_fix ) {
  if (Array.isArray(objdump)) {
    //let t = { id: parent_path, children: objdump.map( r => gen(r) ) }
    let children = {}
    objdump.forEach( r => children[r.$name] = r)
    let newobjdump = { $name: "root", children: children, features:[], features_list:[], links:[], params: {}, $elk_parent_objdump: parent_object }
    let res = gen( newobjdump, parent_path, parent_object )
    if (!skip_links_fix)
  ...
  var t = { id: id, width: 30, height: 30, labels:[], ports: [], children: [], edges: [] };
  ...
  */
  ...
  // именованных тож
  for (let k of Object.keys( objdump.params )) {
    if (parseInt(k).toString() === k) continue;
    if (k == "args_count") continue;
    //console.warn(k)
    let val = objdump.params[k]
    if (Array.isArray(val) && val[0].features) {
      // это у нас {}-случай
      let children = {}
      val.forEach( r => children[r.$name] = r)
      let newobjdump = { $name: "param_"+k, children: children, features:[], features_list:[], links:[], params: {}, $elk_parent_objdump: objdump }
      let res = gen( newobjdump, parent_path, parent_object )
      t.children.push( res )

      // t.children.push( gen( val, id, objdump, false, true ) )
      //objdump.params_env_list = objdump.params_env_list.concat( val )
      objdump.params_env_list[k] = val
    }
    else
    {
      let v = JSON.stringify( objdump.params[k] )
...
*/ 
  ...
  // уберем компутер вообще - заменим на содержимое
  if (objdump.features.computer) {
